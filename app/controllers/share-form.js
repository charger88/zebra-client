define(['helpers'], function (helpers) {
	const submitForm = (e) => {
		...
		localStorage.setItem("share-form-burn", data["burn"]);
		...
		helpers.ajax("POST", window.apiURL + "/stripe", data, [], (response) => {
			...
			window.location.hash = "#open/" + helpers.deFormatCode(response.key) + (false ? "/password" : "");
		});
	};
    ...
    return {
	    ...
	    render: (context) => {
	    	...
	    	if (localStorage.getItem("share-form-burn")){
